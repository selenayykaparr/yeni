...
function loadSavedMember() {
  try {
    const savedMember = JSON.parse(localStorage.getItem(MOLLY_MEMBER_KEY));
...
if (registerForm) {
  registerForm.addEventListener("submit", (e) => {
    ...
    // Üye olunca üstteki "Üye Ol" yazısını kaldır, sadece anahtar kalsın
    if (registerTriggerBtn) {
      registerTriggerBtn.innerHTML = "🔑";
    }

    // Üyelik bilgisini bu cihazda sakla.
    // Böylece uygulama kapanıp yeniden açıldığında üyeyi hatırlar.
    ...
    // Bilgileri Firebase Firestore'daki 'users' koleksiyonuna kaydediyoruz
    ...
    .then(() => {
      console.log("Kullanıcı başarıyla Firebase'e kaydedildi! 🎉");
    })
    .catch((error) => {
