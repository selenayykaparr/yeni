...
function showOfflineMessage() {
  const existingNotice = document.getElementById("molly-offline-notice");
  ...
  overlay.id = "molly-offline-notice";
  ...
  box.style.border = "3px solid #00bcd4";
  box.style.borderRadius = "24px";
  box.style.padding = "24px 20px 18px";
  box.style.boxSizing = "border-box";
  box.style.textAlign = "center";
  box.style.boxShadow = "0 12px 35px rgba(0, 0, 0, 0.22)";
  ...
  title.textContent = "İnternet bağlantısı yok";
  title.style.fontSize = "19px";
  title.style.fontWeight = "800";
  title.style.color = "#00a9c0";
...
function playBook(book) {
  // İnternet yoksa masalı açma, Molly.co'nun kendi uyarısını göster
  if (!navigator.onLine) {
    showOfflineMessage();
    return;
  }
  ...
  playingDesc.textContent =
    `Süre: ${book.duration} - Keyifli dinlemeler!`;
...
mainAudio.addEventListener("timeupdate", () => {
  if (mainAudio.duration) {
    ...
    const currentMinutes =
      Math.floor(mainAudio.currentTime / 60);
...
if (favoriteBtn) {
  favoriteBtn.addEventListener("click", () => {
    ...
    const index =
      favorites.indexOf(currentBook.id);
...
if (registerTriggerBtn) {
  registerTriggerBtn.addEventListener(
    "click",
    () => {
...
function loadSavedMember() {
  try {
    ...
    console.error(
      "Kayıtlı üyelik bilgisi okunamadı:",
      error
    );
  }
}
